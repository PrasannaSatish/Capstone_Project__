...
(
    ...
    always @(posedge clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            clause_no    <= 0;        // FIX: was 1 (preset FF on bit[0]), now 0
            clause_idx   <= 0;
            should_add   <= 0;
            should_add_r <= 0;
            ip_done_reg  <= 0;
            ...
            if (reset) begin
                clause_no    <= 1;        // sync reset keeps clause_no=1  UNCHANGED
                clause_idx   <= 0;
                should_add   <= 0;
                should_add_r <= 0;
                ip_done_reg  <= 0;
    ...
    //
    // FIX (MAP-2): max_sum async reset changed from -1000 to 18'sh0.
    // -1000 in 18-bit two's complement has many bits set to 1  preset FFs.
    // Sync reset (reset branch) still sets max_sum = -1000  UNCHANGED.
    // max_sum is only read during done_conv_long which only fires after
    // the synchronous reset has completed, so functionality is preserved.
    // ------------------------------------------------------------
    always @(posedge clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            max_sum  <= 18'sh0;   // FIX: was -1000 (many preset FFs), now 0
            kdx      <= 0;
            class_op <= 0;
            done     <= 0;
        end
            ...
            if (reset) begin
                max_sum  <= -18'sd1000;   // sync reset keeps -1000  UNCHANGED
                kdx      <= 0;
                class_op <= 0;
                done     <= 0;
            end
